...
const forceShowPopover = (options) => absoluteRoot(options).find('.ant-popover').then($el => $el.css({ 'pointer-events': 'all', opacity: 1 }))
const forceHidePopover = (options) => absoluteRoot(options).find('.ant-popover').then($el => $el.css({ 'pointer-events': 'none', opacity: 0 }))
